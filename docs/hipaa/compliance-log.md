...
- **Credential remediation:** Inspection found a previously committed Neon runtime-role password in scripts/neon-runtime-role-setup.sql. The tracked value was replaced with a fail-closed placeholder. The user confirmed separate Dev and UAT runtime-role password rotations and corresponding local/Azure connection-setting updates before merge. The Neon production branch does not contain the affected role.
